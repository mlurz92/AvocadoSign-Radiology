window.utils = (() => {
    ...
    function getPValueText(pValue, includeLeadingZero = true) {
        ...
        if (parseFloat(formattedP) >= 0.05 && p < 0.05) {
            formattedP = p.toFixed(3);
        }
        
        if (parseFloat(formattedP) > 0.99) {
            return `${prefix}>.99`;
        }
    ...
    return Object.freeze({
        ...
        getSortFunction,
        getStatisticalSignificanceSymbol,
        getPValueText,
        generateUUID,
        clampNumber,
        getAUCInterpretation,
        getPhiInterpretation,
        getORInterpretation,
        escapeHTML,
        getDefinitionTooltip,
        getInterpretationTooltip,
        getT2IconSVG,
        formatStatusBadge
    });
})();
