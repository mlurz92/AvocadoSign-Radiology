    ...
    function createCriteriaComparisonTableHTML(allStats, globalCoh, appliedCriteria, appliedLogic) {
        ...
        results.forEach(r => {
            ...
            tableHtml += `<tr>
                ...
                <td>${window.utils.formatPercent(r.sens?.value, 1, na_stat)}</td>
                <td>${window.utils.formatPercent(r.spec?.value, 1, na_stat)}</td>
                <td>${window.utils.formatPercent(r.ppv?.value, 1, na_stat)}</td>
                <td>${window.utils.formatPercent(r.npv?.value, 1, na_stat)}</td>
                <td>${window.utils.formatPercent(r.acc?.value, 1, na_stat)}</td>
                <td>${window.utils.formatNumber(r.auc?.value, 3, na_stat, true)}</td>
                <td data-tippy-content="${pValueTooltip}">${r.pValue ? `${window.utils.getPValueText(r.pValue, false)} ${window.utils.getStatisticalSignificanceSymbol(r.pValue)}` : na_stat}</td>
            </tr>`;
        });
    ...
    function render(processedData, appliedCriteria, appliedLogic, layout, cohort1, cohort2, globalCohort) {
        ...
        cohortIdsToShow.forEach((cohortId, i) => {
            ...
            if (stats && stats.descriptive.patientCount > 0) {
                ...
                const fCI_p_stat = (m, k) => { const d = (k === 'auc' || k ==='f1' || k==='youden') ? 3 : 1; const p = !(k === 'auc'||k==='f1'||k==='youden'); return window.utils.formatCI(m?.value, m?.ci?.lower, m?.ci?.upper, d, p, na_stat); };
                ...
                const createPerfTableHTML = (perfStats) => {
                    ...
                    return `<div class="table-responsive"><table class="table table-sm table-striped small mb-0"><thead><tr>
                        ...
                        <tr><td data-tippy-content="${window.utils.getDefinitionTooltip('sens')}">Sensitivity</td><td data-tippy-content="${window.utils.getInterpretationTooltip('sens', perfStats.sens)}">${fCI_p_stat(perfStats.sens, 'sens')}</td><td>${perfStats.sens?.method || na_stat}</td></tr>
                        <tr><td data-tippy-content="${window.utils.getDefinitionTooltip('spec')}">Specificity</td><td data-tippy-content="${window.utils.getInterpretationTooltip('spec', perfStats.spec)}">${fCI_p_stat(perfStats.spec, 'spec')}</td><td>${perfStats.spec?.method || na_stat}</td></tr>
                        <tr><td data-tippy-content="${window.utils.getDefinitionTooltip('ppv')}">PPV</td><td data-tippy-content="${window.utils.getInterpretationTooltip('ppv', perfStats.ppv)}">${fCI_p_stat(perfStats.ppv, 'ppv')}</td><td>${perfStats.ppv?.method || na_stat}</td></tr>
                        <tr><td data-tippy-content="${window.utils.getDefinitionTooltip('npv')}">NPV</td><td data-tippy-content="${window.utils.getInterpretationTooltip('npv', perfStats.npv)}">${fCI_p_stat(perfStats.npv, 'npv')}</td><td>${perfStats.npv?.method || na_stat}</td></tr>
                        <tr><td data-tippy-content="${window.utils.getDefinitionTooltip('acc')}">Accuracy</td><td data-tippy-content="${window.utils.getInterpretationTooltip('acc', perfStats.acc)}">${fCI_p_stat(perfStats.acc, 'acc')}</td><td>${perfStats.acc?.method || na_stat}</td></tr>
                ...
                const createCompTableHTML = (compStats) => {
                    ...
                    return `<div class="table-responsive"><table class="table table-sm table-striped small mb-0"><thead><tr>
                        ...
                        <tr><td data-tippy-content="${window.utils.getDefinitionTooltip('mcnemar')}">McNemar (Acc)</td><td>${window.utils.formatNumber(compStats.mcnemar?.statistic, 3, na_stat, true)} (df=${compStats.mcnemar?.df || na_stat})</td><td data-tippy-content="${mcnemarTooltip}">${window.utils.getPValueText(compStats.mcnemar?.pValue, false)} ${window.utils.getStatisticalSignificanceSymbol(compStats.mcnemar?.pValue)}</td><td>${compStats.mcnemar?.method || na_stat}</td></tr>
                        <tr><td data-tippy-content="${window.utils.getDefinitionTooltip('delong')}">DeLong (AUC)</td><td>Z=${window.utils.formatNumber(compStats.delong?.Z, 3, na_stat, true)}</td><td data-tippy-content="${delongTooltip}">${window.utils.getPValueText(compStats.delong?.pValue, false)} ${window.utils.getStatisticalSignificanceSymbol(compStats.delong?.pValue)}</td><td>${compStats.delong?.method || na_stat}</td></tr>
                    </tbody></table></div>`;
                };
                ...
                const createAssocTableHTML = (assocStats, appliedCrit) => {
                    ...
                    const addRow = (key, name, obj) => {
                        ...
                        html += `<tr><td>${window.utils.escapeHTML(name)}</td>
                            ...
                            <td data-tippy-content="${pValueTooltip}">${window.utils.getPValueText(obj.pValue, false)} ${window.utils.getStatisticalSignificanceSymbol(obj.pValue)}</td>
                    ...
                    if (assocStats.size_mwu) {
                        ...
                        html += `<tr><td>${assocStats.size_mwu.featureName}</td><td>${na_stat}</td><td>${na_stat}</td><td>${na_stat}</td><td data-tippy-content="${mwuTooltip}">${window.utils.getPValueText(assocStats.size_mwu.pValue, false)} ${window.utils.getStatisticalSignificanceSymbol(assocStats.size_mwu.pValue)}</td><td>${assocStats.size_mwu.testName || na_stat}</td></tr>`;
                    }
        ...
        if (layout === 'vergleich' && allCohortStats[cohort1] && allCohortStats[cohort2] && allCohortStats.interCohortComparison) {
             ...
             if(interComp.as && interComp.t2Applied) {
                 compContent = `<div class="table-responsive"><table class="table table-sm table-striped small mb-0">
                    ...
                    <tbody>
                        <tr><td>Avocado Sign</td><td>AUC</td><td>${window.utils.formatNumber(c1Stats.performanceAS.auc.value, 3, na_stat, true)}</td><td>${window.utils.formatNumber(c2Stats.performanceAS.auc.value, 3, na_stat, true)}</td><td>${window.utils.getPValueText(interComp.as.pValue, false)}</td></tr>
                        <tr><td>${formattedAppliedT2Short}</td><td>AUC</td><td>${window.utils.formatNumber(c1Stats.performanceT2Applied.auc.value, 3, na_stat, true)}</td><td>${window.utils.formatNumber(c2Stats.performanceT2Applied.auc.value, 3, na_stat, true)}</td><td>${window.utils.getPValueText(interComp.t2Applied.pValue, false)}</td></tr>
                    </tbody>
                 </table></div>`;
             }
