window.methodsGenerator = (() => {
    ...
    function generateStudyDesignHTML(stats, commonData) {
        ...
        return `
            ...
            <p>This retrospective, single-institution study was performed in compliance with the Health Insurance Portability and Accountability Act and received approval from the institutional review board. The requirement for written informed consent was waived for this retrospective analysis.</p>
            <p>We performed a secondary analysis of a previously described cohort of ${helpers.formatValueForPublication(nOverall, 0)} consecutive patients who underwent pelvic MRI for primary staging or restaging of histologically confirmed rectal cancer between January 2020 and November 2023 ${helpers.getReference('Lurz_Schaefer_2025')}. The initial study introduced the Avocado Sign and evaluated its standalone performance. The current study focuses on a direct comparison of the Avocado Sign against T2-weighted criteria. Inclusion criteria were the availability of high-quality T2-weighted and contrast-enhanced T1-weighted MRI sequences and a definitive histopathological reference standard from the subsequent surgical resection specimen. Of the final cohort, ${helpers.formatValueForPublication(nSurgeryAlone, 0)} (${helpers.formatValueForPublication(nSurgeryAlone / nOverall, 0, true)}%) underwent primary surgery after initial staging, while ${helpers.formatValueForPublication(nNeoadjuvantTherapy, 0)} (${helpers.formatValueForPublication(nNeoadjuvantTherapy / nOverall, 0, true)}%) received neoadjuvant chemoradiotherapy and underwent subsequent restaging MRI.</p>
        `;
    }
    ...
    function generateStatisticalAnalysisHTML(stats, commonData) {
        ...
        return `
            ...
            <p>Descriptive statistics were used to summarize patient characteristics. Diagnostic performance metrics—including sensitivity, specificity, positive predictive value, negative predictive value, and accuracy—were calculated for each diagnostic method. Wilson score method was used for 95% confidence intervals (CIs) of proportions, and the bootstrap percentile method (${window.APP_CONFIG.STATISTICAL_CONSTANTS.BOOTSTRAP_CI_REPLICATIONS} replications) was used for CIs of the area under the receiver operating characteristic curve (AUC) and F1-score.</p>
            <p>The primary comparison between the AUC of the Avocado Sign and the cohort-optimized T2w criteria was performed using the method described by DeLong et al. for correlated ROC curves. McNemar’s test was used to compare accuracies. For associations between individual categorical features and N-status, Fisher's exact test was used. All statistical analyses were performed using custom software scripts (JavaScript, ES2020+) implemented in the analysis tool itself (Version ${window.APP_CONFIG.APP_VERSION}). A two-sided ${helpers.formatPValueForPublication(statisticalSignificanceLevel)} was considered to indicate statistical significance.</p>
        `;
    }
