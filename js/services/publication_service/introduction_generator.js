window.introductionGenerator = (() => {
    ...
    function generateIntroductionHTML(stats, commonData) {
        ...
        const introText = `
            ...
            <p>Multiple studies and meta-analyses have highlighted the suboptimal diagnostic performance of these T2w criteria. A meta-analysis by Al-Sukhni et al found a pooled sensitivity and specificity of only 77% and 71%, respectively. More recent validations of the complex, size-dependent ESGAR consensus criteria reported a sensitivity as low as 54%. This diagnostic uncertainty can lead to both over- and undertreatment, underscoring a critical need for more reliable imaging markers. While advanced techniques like diffusion-weighted imaging have been explored, they have not yet consistently surpassed morphological assessment in clinical practice.</p>
            <p>Our group previously introduced the "Avocado Sign," a marker observed on contrast-enhanced T1-weighted MRI, defined as a distinct hypointense core within a homogeneously hyperintense lymph node. The initial evaluation suggested high diagnostic accuracy and excellent interobserver reproducibility. However, a direct and robust comparison against both established literature-based criteria and a data-optimized "best-case" scenario for T2w morphology on the same patient cohort has been lacking.</p>
