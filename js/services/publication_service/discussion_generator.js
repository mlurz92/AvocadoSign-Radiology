const discussionGenerator = (() => {
    ...
    function generateDiscussionHTML(stats, commonData) {
        ...
        let discussionText = `
            ...
            <p>In this study, we demonstrated that the Avocado Sign, a contrast-enhancement-based MRI marker, has high diagnostic performance for predicting mesorectal lymph node metastasis, with an overall accuracy of ${helpers.formatMetricForPublication(performanceAS.acc, 'acc')} and an AUC of ${helpers.formatMetricForPublication(performanceAS.auc, 'auc')}. Our findings indicate that the Avocado Sign not only outperforms established literature-based T2w criteria but also shows a diagnostic performance statistically non-inferior to that of a cohort-optimized T2w criteria set, which was computationally derived to represent a "best-case" scenario for conventional morphology${bfResultsAvailable ? ` (${helpers.formatPValueForPublication(comparisonASvsT2Bruteforce.delong.pValue)} for AUC comparison)` : ''}. This suggests that the Avocado Sign is a robust and simpler alternative to complex morphological assessments.</p>
            <p>The limitations of conventional T2w criteria are well-documented. Prior meta-analyses have reported suboptimal pooled sensitivity and specificity of 77% and 71%, respectively ${helpers.getReference('Al_Sukhni_2012')}. Even more refined, modern criteria like the ESGAR 2016 consensus guidelines have shown limited sensitivity in recent validations ${helpers.getReference('Rutegard_2025')}. Our results align with these findings, showing that literature-based criteria performed variably and often less effectively than the Avocado Sign. The Avocado Sign, in contrast, offers a potential simplification by providing a single, reproducible binary feature that appears effective across both primary staging and post-treatment evaluation scenarios. This is supported by its almost perfect interobserver agreement in our study (Cohen’s kappa = ${helpers.formatMetricForPublication(overallStats.interobserverKappa, 'kappa')}${(overallStats.interobserverKappaCI && isFinite(overallStats.interobserverKappaCI.lower) && isFinite(overallStats.interobserverKappaCI.upper)) ? `; 95% CI: ${helpers.formatValueForPublication(overallStats.interobserverKappaCI.lower, 2)}, ${helpers.formatValueForPublication(overallStats.interobserverKappaCI.upper, 2)}` : ''}), which suggests that the sign can be reliably assessed by radiologists with varying levels of expertise. Its straightforward application, requiring only routine high-resolution, fat-saturated contrast-enhanced T1-weighted sequences, makes it easily incorporable into standard MRI protocols.</p>
            <p>This study had several limitations. First, its retrospective, single-center design may limit the generalizability of our findings, and selection bias cannot be entirely ruled out. The analysis was performed on a previously described patient cohort, which may introduce its own biases. Second, all MRI examinations were performed on a single 3.0-T MRI system using one type of macrocyclic gadolinium-based contrast agent (Gadoteridol); thus, performance with other agents or at different field strengths is unknown and requires further validation. Third, our analysis was performed on a per-patient basis rather than a node-by-node correlation, which is challenging after neoadjuvant therapy and of debated clinical utility. Finally, this analysis did not include extramesorectal nodes or assess long-term oncologic outcomes, which warrants future research.</p>
