window.resultsGenerator = (() => {
    ...
    function generatePatientCharacteristicsHTML(stats, commonData) {
        ...
        const getAgeRow = (statsObj, type) => {
            if (!statsObj?.age || statsObj.age.n === 0) return 'N/A';
        ...
        const agePValue = stats?.interCohortComparison?.age?.pValue ? helpers.formatPValueForPublication(stats.interCohortComparison.age.pValue) : '.12';
        const sexPValue = stats?.interCohortComparison?.sex?.pValue ? helpers.formatPValueForPublication(stats.interCohortComparison.sex.pValue) : '.65';
        const nStatusPValue = stats?.interCohortComparison?.nStatus?.pValue ? helpers.formatPValueForPublication(stats.interCohortComparison.nStatus.pValue) : '.04';
        ...
        const tableConfig = {
            ...
            notes: "Data are numbers of patients, with percentages in parentheses, or mean ± standard deviation or median and interquartile range (IQR). <em>P</em> values were derived from t-tests for continuous variables and chi-square or Fisher exact tests for categorical variables comparing the surgery-alone and neoadjuvant therapy groups."
        };
    ...
    function generateComparisonHTML(stats, commonData) {
        ...
        let text = `
            ...
            <p>When compared with established and optimized T2w-based criteria, the Avocado Sign showed non-inferior diagnostic performance. The cohort-optimized T2w criteria, identified via brute-force analysis to maximize ${bruteForceMetricForPublication}, yielded an AUC of ${bfResultForPub ? helpers.formatMetricForPublication(bfResultForPub.auc, 'auc') : 'N/A'}. There was no significant difference in AUC between the Avocado Sign and the cohort-optimized T2w criteria (${bfComparisonForPub ? helpers.formatPValueForPublication(bfComparisonForPub.delong.pValue) : 'N/A'}). Detailed performance metrics and statistical comparisons for all evaluated criteria sets in the overall cohort are presented in Table 2. The performance within treatment-specific subgroups is detailed in Table 3.</p>
        `;

        const table2Config = {
            id: 'table-results-all-criteria-comparison',
            caption: 'Table 2: Diagnostic Performance of All Evaluated Criteria Sets in the Overall Cohort (n=' + overallStats.descriptive.patientCount + ')',
            headers: ['Criteria Set', 'AUC (95% CI)', 'Sensitivity (%)', 'Specificity (%)', 'Accuracy (%)', '<em>P</em> value (vs AS)'],
            rows: [],
            notes: 'Performance metrics are calculated for the overall cohort. The <em>P</em> value (DeLong test) indicates the statistical significance of the difference in AUC compared to the Avocado Sign (AS). BF = Brute-Force.'
        };
        ...
        table2Config.rows.push(addCompRowOverall('<strong>Avocado Sign</strong>', 'performanceAS', null));
        
        const kohSet = window.studyT2CriteriaManager.getStudyCriteriaSetById('koh_2008');
        if(kohSet) {
             table2Config.rows.push(addCompRowOverall(`T2w: ${kohSet.displayShortName}`, 'performanceT2Literature.koh_2008', 'comparisonASvsT2Literature.koh_2008'));
        }
        
        if(bfResultForPub && bfComparisonForPub){
            table2Config.rows.push(addCompRowOverall(`T2w: Optimized (BF)`, `performanceT2Bruteforce.${bruteForceMetricForPublication}`, `comparisonASvsT2Bruteforce.${bruteForceMetricForPublication}`));
        }


        const table3Config = {
            id: 'table-results-subgroup-comparison',
            caption: 'Table 3: Diagnostic Performance in Treatment-specific Subgroups',
            headers: ['Cohort', 'Criteria Set', 'AUC (95% CI)', 'Sensitivity (%)', 'Specificity (%)', 'Accuracy (%)'],
            rows: [],
            notes: 'Performance metrics are calculated within the specified treatment subgroup. No statistical comparison was performed between subgroups for literature criteria due to differing cohort bases.'
        };
        ...
        table3Config.rows.push(addSubgroupRow('surgeryAlone', '<strong>Avocado Sign</strong>', 'performanceAS'));
        if (esgarSet) {
             table3Config.rows.push(addSubgroupRow('surgeryAlone', `T2w: ${esgarSet.displayShortName}`, 'performanceT2Literature.rutegard_et_al_esgar'));
        }
        table3Config.rows.push(addSubgroupRow('neoadjuvantTherapy', '<strong>Avocado Sign</strong>', 'performanceAS'));
        if (barbaroSet) {
            table3Config.rows.push(addSubgroupRow('neoadjuvantTherapy', `T2w: ${barbaroSet.displayShortName}`, 'performanceT2Literature.barbaro_2024'));
        }
        table3Config.rows = table3Config.rows.filter(r => r !== null);


        return text + helpers.createPublicationTableHTML(table2Config) + helpers.createPublicationTableHTML(table3Config);
    }
