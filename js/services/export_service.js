window.exportService = (() => {
    ...
    function generateFilename(typeKey, kollektiv, extension, options = {}) {
        const dateStr = window.utils.getCurrentDateString(window.APP_CONFIG.EXPORT_SETTINGS.DATE_FORMAT);
        const safeKollektiv = window.utils.getCohortDisplayName(kollektiv).replace(/[^a-z0-9_-]/gi, '_').replace(/_+/g, '_');
        ...
        if (options.studyId && filenameType.includes('{StudyID}')) {
             ...
             filenameType = filename.replace('_{StudyID}', '').replace('{StudyID}', '');
        }
        ...
        if (options.sectionName && filenameType.includes('{SectionName}')) {
            ...
            filenameType = filename.replace('_{SectionName}', '').replace('{SectionName}', '');
        }
    ...
    function generateStatistikCSVString(stats, kollektiv, criteria, logic) {
        ...
        const fv = (v, d, p = false) => window.utils.formatNumber(v, d, na, p);
        const fp = (v, d) => window.utils.formatPercent(v, d, na);
        ...
        try {
            ...
            csvData.push(['Kollektiv', window.utils.getCohortDisplayName(kollektiv)]); 
            ...
            csvData.push(['Accuracy (McNemar)', fv(v?.mcnemar?.statistic, 3, true), window.utils.getPValueText(v?.mcnemar?.pValue, false), v?.mcnemar?.method || na]); 
            csvData.push(['AUC (DeLong)', fv(v?.delong?.Z, 3, true), window.utils.getPValueText(v?.delong?.pValue, false), v?.delong?.method || na]); 
            csvData.push([]);
            ...
            const addAssocRow = (key, name, obj) => { if (!obj) return; const orCI = fCI(obj.or?.ci, 2, false); const rdCI = fCI(obj.rd?.ci, 3, false); csvData.push([ key, name, fv(obj.or?.value, 2, true), orCI[0], orCI[1], fv(obj.rd?.value, 3, true), rdCI[0], rdCI[1], fv(obj.phi?.value, 2, true), fv(obj.statistic ?? NaN, 2, true), window.utils.getPValueText(obj.pValue, false), obj.testName || na ]); }; 
            const a = stats.associationsApplied; 
            addAssocRow('as', 'AS Positiv', a?.as); 
            if(a?.size_mwu) { csvData.push(['size_mwu', a.size_mwu.featureName || 'LK Größe MWU', na, na, na, na, na, na, na, fv(a.size_mwu.Z, 2, true), window.utils.getPValueText(a.size_mwu.pValue, false), a.size_mwu.testName || na ]); } 
    ...
    function generateBruteForceTXTString(resultsData) {
        ...
        try {
            ...
            const cohortName = window.utils.getCohortDisplayName(cohort);
            ...
            report += `   - Total Patients: ${window.utils.formatNumber(nTotal, 0, na)}\r\n`;
            report += `   - N+ Patients: ${window.utils.formatNumber(nPlus, 0, na)}\r\n`;
            report += `   - N- Patients: ${window.utils.formatNumber(nMinus, 0, na)}\r\n`;
            report += `Optimized Target Metric: ${metric}\r\n`;
            report += `Total Duration: ${window.utils.formatNumber((duration || 0) / 1000, 1, na, true)} seconds\r\n`;
            report += `Combinations Tested: ${window.utils.formatNumber(totalTested, 0, na, true)}\r\n`;
            report += `==================================================\r\n\r\n`;
            ...
            if (bestResult) {
                ...
                report += `Achieved ${metric}: ${window.utils.formatNumber(bestResult.metricValue, 4, na, true)}\r\n\r\n`;
            }
            ...
            for (let i = 0; i < results.length; i++) {
                ...
                report += `${String(currentRank).padEnd(4)} | ${window.utils.formatNumber(result.metricValue, 4, na, true).padEnd(12)} | ${result.logic.toUpperCase().padEnd(5)} | ${formatCriteriaFunc(result.criteria, result.logic)}\r\n`;
    ...
    function generateMarkdownTableString(dataOrStats, tableType, kollektiv, criteria = null, logic = null, options = {}) {
        try {
            ...
            const kollektivDisplayName = window.utils.getCohortDisplayName(kollektiv); 
            const escMD = (text) => window.utils.escapeHTML(String(text ?? '')); 
            const na = window.APP_CONFIG.NA_PLACEHOLDER;
            const formatCriteriaFunc = typeof window.studyT2CriteriaManager !== 'undefined' ? window.studyT2CriteriaManager.formatCriteriaForDisplay : (c, l) => 'N/A'; 
            ...
            if (tableType === 'daten') { title = 'Data List'; headers = ['ID', 'Last Name', 'First Name', 'Sex', 'Age', 'Therapy', 'N', 'AS', 'T2', 'Notes']; if(!Array.isArray(dataOrStats)) return `# ${title}...\n\nError: Invalid data.`; rows = dataOrStats.map(p => [p.id, p.lastName || '', p.firstName || '', p.sex || '', p.age ?? '', window.utils.getCohortDisplayName(p.therapy), p.nStatus ?? na, p.asStatus ?? na, p.t2Status ?? na, p.notes || ''].map(escMD)); }
            else if (tableType === 'auswertung') { title = 'Analysis Table'; headers = ['ID', 'Name', 'Therapy', 'N', 'AS', 'T2', 'N+/N total', 'AS+/AS total', 'T2+/T2 total']; if(!Array.isArray(dataOrStats)) return `# ${title}...\n\nError: Invalid data.`; rows = dataOrStats.map(p => [p.id, p.lastName || '', window.utils.getCohortDisplayName(p.therapy), p.nStatus ?? na, p.asStatus ?? na, p.t2Status ?? na, `${window.utils.formatNumber(p.countPathologyNodesPositive, 0, na)} / ${window.utils.formatNumber(p.countPathologyNodes, 0, na)}`, `${window.utils.formatNumber(p.countASNodesPositive, 0, na)} / ${window.utils.formatNumber(p.countASNodes, 0, na)}`, `${window.utils.formatNumber(p.countT2NodesPositive, 0, na)} / ${window.utils.formatNumber(p.countT2Nodes, 0, na)}`].map(escMD)); }
            else if (tableType === 'deskriptiv') { title = 'Descriptive Statistics'; const stats = dataOrStats; if (!stats || !stats.patientCount) return `# ${title} (Cohort: ${kollektivDisplayName})\n\nNo data available.`; const total = stats.patientCount; headers = ['Metric', 'Value']; const fLKRowMD = (lk) => `${window.utils.formatNumber(lk?.median, 1, na)} (${window.utils.formatNumber(lk?.min, 0, na)}-${window.utils.formatNumber(lk?.max, 0, na)}) \\[Mean: ${window.utils.formatNumber(lk?.mean, 1, na)} ± ${window.utils.formatNumber(lk?.sd, 1, na)}\\]`; rows = [ ['Number of Patients', total], ['Median Age (Min-Max) \\[Mean ± SD\\]', `${window.utils.formatNumber(stats.age?.median, 1, na)} (${window.utils.formatNumber(stats.age?.min, 0, na)} - ${window.utils.formatNumber(stats.age?.max, 0, na)}) \\[${window.utils.formatNumber(stats.age?.mean, 1, na)} ± ${window.utils.formatNumber(stats.age?.sd, 1, na)}\\]`], ['Sex (m/f) (n / %)', `${stats.sex?.m ?? 0} / ${stats.sex?.f ?? 0} (${window.utils.formatPercent((stats.sex?.m ?? 0) / total, 1)} / ${window.utils.formatPercent((stats.sex?.f ?? 0) / total, 1)})`], ['Therapy (Upfront Surgery / nRCT) (n / %)', `${stats.therapy?.surgeryAlone ?? 0} / ${stats.therapy?.neoadjuvantTherapy ?? 0} (${window.utils.formatPercent((stats.therapy?.surgeryAlone ?? 0) / total, 1)} / ${window.utils.formatPercent((stats.therapy?.neoadjuvantTherapy ?? 0) / total, 1)})`], ['N Status (+ / -) (n / %)', `${stats.nStatus?.plus ?? 0} / ${stats.nStatus?.minus ?? 0} (${window.utils.formatPercent((stats.nStatus?.plus ?? 0) / total, 1)} / ${window.utils.formatPercent((stats.nStatus?.minus ?? 0) / total, 1)})`], ['AS Status (+ / -) (n / %)', `${stats.asStatus?.plus ?? 0} / ${stats.asStatus?.minus ?? 0} (${window.utils.formatPercent((stats.asStatus?.plus ?? 0) / total, 1)} / ${window.utils.formatPercent((stats.asStatus?.minus ?? 0) / total, 1)})`], ['T2 Status (+ / -) (n / %)', `${stats.t2Status?.plus ?? 0} / ${stats.t2Status?.minus ?? 0} (${window.utils.formatPercent((stats.t2Status?.plus ?? 0) / total, 1)} / ${window.utils.formatPercent((stats.t2Status?.minus ?? 0) / total, 1)})`], ['Median LN N total (Min-Max) \\[Mean ± SD\\]', fLKRowMD(stats.lnCounts?.n?.total)], ['Median LN N+ (Min-Max) \\[Mean ± SD\\] (only N+ Pat.)', fLKRowMD(stats.lnCounts?.n?.plus)], ['Median LN AS total (Min-Max) \\[Mean ± SD\\]', fLKRowMD(stats.lnCounts?.as?.total)], ['Median LN AS+ (Min-Max) \\[Mean ± SD\\] (only AS+ Pat.)', fLKRowMD(stats.lnCounts?.as?.plus)], ['Median LN T2 total (Min-Max) \\[Mean ± SD\\]', fLKRowMD(stats.lnCounts?.t2?.total)], ['Median LN T2+ (Min-Max) \\[Mean ± SD\\] (only T2+ Pat.)', fLKRowMD(stats.lnCounts?.t2?.plus)] ].map(r => r.map(escMD)); }
            else if (tableType === 'comp_as_perf') { title = `Diagnostic Performance (AS) for Cohorts`; const { statsGesamt, statsSurgeryAlone, statsNeoadjuvantTherapy } = dataOrStats || {}; if (!statsGesamt && !statsSurgeryAlone && !statsNeoadjuvantTherapy) return `# ${title}\n\nError: Invalid data.`; headers = ['Cohort', 'Sens. (95% CI)', 'Spec. (95% CI)', 'PPV (95% CI)', 'NPV (95% CI)', 'Acc. (95% CI)', 'AUC (95% CI)']; const fRow = (s, k) => { const d = window.utils.getCohortDisplayName(k); if (!s || typeof s.matrix !== 'object') return [d + ' (N=?)', na, na, na, na, na, na].map(escMD); const n = s.matrix ? (s.matrix.tp + s.matrix.fp + s.matrix.fn + s.matrix.tn) : 0; const fCI_p = (m, ky) => { const dig = (ky === 'auc') ? 3 : ((ky === 'f1') ? 3 : 1); const isP = !(ky === 'f1' || ky === 'auc'); return window.utils.formatCI(m?.value, m?.ci?.lower, m?.ci?.upper, dig, isP, na); }; return [`${d} (N=${n})`, fCI_p(s.sens, 'sens'), fCI_p(s.spec, 'spec'), fCI_p(s.ppv, 'ppv'), fCI_p(s.npv, 'npv'), fCI_p(s.acc, 'acc'), fCI_p(s.auc, 'auc')].map(escMD); }; rows = [ fRow(statsGesamt, 'Overall'), fRow(statsSurgeryAlone, 'surgeryAlone'), fRow(statsNeoadjuvantTherapy, 'neoadjuvantTherapy') ]; }
            else if (tableType === 'comp_as_vs_t2_perf' || tableType === 'comp_as_vs_t2_comp') { const { performanceAS, performanceT2 } = dataOrStats || {}; title = `Comparison Diagnostic Performance (AS vs. ${escMD(t2CriteriaLabelShort)})`; if (!performanceAS || !performanceT2) return `# ${title} (Cohort: ${kollektivDisplayName})\n\nError: Invalid data for comparison.`; headers = ['Metric', 'AS (Value, 95% CI)', `${escMD(t2CriteriaLabelShort)} (Value, 95% CI)`]; const fRow = (mKey, nm, isP = true, d = 1) => { const mAS = performanceAS[mKey]; const mT2 = performanceT2[mKey]; const dig = (mKey === 'auc' || mKey === 'f1' || mKey === 'balAcc') ? 3 : d; const vAS = window.utils.formatCI(mAS?.value, mAS?.ci?.lower, mAS?.ci?.upper, dig, isP, na); const vT2 = window.utils.formatCI(mT2?.value, mT2?.ci?.lower, mT2?.ci?.upper, dig, isP, na); return [nm, vAS, vT2]; }; rows = [ fRow('sens', 'Sensitivity'), fRow('spec', 'Specificity'), fRow('ppv', 'PPV'), fRow('npv', 'NPV'), fRow('acc', 'Accuracy'), fRow('balAcc', 'Balanced Accuracy', false, 3), fRow('f1', 'F1-Score', false, 3), fRow('auc', 'AUC', false, 3) ].map(r => r.map(escMD)); }
            else if (tableType === 'comp_as_vs_t2_tests') { const { comparison } = dataOrStats || {}; title = `Statistical Comparison (AS vs. ${escMD(t2CriteriaLabelShort)})`; if (!comparison) return `# ${title} (Cohort: ${kollektivDisplayName})\n\nError: Invalid data for comparison tests.`; headers = ['Test', 'Statistic Value', 'p-Value', 'Method']; rows = [ ['McNemar (Accuracy)', `${window.utils.formatNumber(comparison?.mcnemar?.statistic, 3, na, true)} (df=${comparison?.mcnemar?.df || na})`, `${window.utils.getPValueText(comparison?.mcnemar?.pValue, true)} ${window.utils.getStatisticalSignificanceSymbol(comparison?.mcnemar?.pValue)}`, `${comparison?.mcnemar?.method || na}`], ['DeLong (AUC)', `Z=${window.utils.formatNumber(comparison?.delong?.Z, 3, na, true)}`, `${window.utils.getPValueText(comparison?.delong?.pValue, true)} ${window.utils.getStatisticalSignificanceSymbol(comparison?.delong?.pValue)}`, `${window.utils.delong?.method || na}`] ].map(r => r.map(escMD)); }
   ...
   function generateFilteredDataCSVString(data) {
    ...
    function generateComprehensiveReportHTML(data, allBruteForceResults, kollektiv, criteria, logic) {
        try {
            ...
            const formatCriteriaFunc = typeof window.studyT2CriteriaManager !== 'undefined' ? window.studyT2CriteriaManager.formatCriteriaForDisplay : (c, l) => 'N/A';
            const kollektivName = window.utils.getCohortDisplayName(kollektiv); const timestamp = window.utils.getCurrentDateString(null) + ' ' + new Date().toLocaleTimeString('en-US'); const criteriaString = formatCriteriaFunc(criteria, logic); 
            const appliedCriteriaDisplayName = `Applied T2 (${formatCriteriaFunc(criteria, logic, true)})`;
            
            const getChartSVG = (element) => { if(!element) return `<p class="text-muted small">[Chart element not found]</p>`; try { const clone = element.cloneNode(true); clone.setAttribute('xmlns', 'http://www.w3.org/2000/svg'); clone.setAttribute('version', '1.1'); clone.style.backgroundColor = window.APP_CONFIG.CHART_SETTINGS.PLOT_BACKGROUND_COLOR || '#ffffff'; const styles = window.getComputedStyle(element); const viewBox = element.viewBox?.baseVal; let w = element.getAttribute('width') || styles.width || viewBox?.width, h = element.getAttribute('height') || styles.height || viewBox?.height; if (vb && vb.length === 4 && parseFloat(vb[2]) > 0 && parseFloat(vb[3]) > 0) { clone.setAttribute('width', vb[2]); clone.setAttribute('height', vb[3]); } else if (!w || !h || parseFloat(w) <= 0 || parseFloat(h) <= 0) { clone.setAttribute('width', '400'); clone.setAttribute('height', '300'); } const styleEl = document.createElementNS("http://www.w3.org/2000/svg", "style"); styleEl.textContent = `svg { font-family: ${window.getComputedStyle(document.body).fontFamily || 'sans-serif'}; } .axis path, .axis line { fill: none; stroke: #6c757d; shape-rendering: crispEdges; stroke-width: 1px; } .axis text { font-size: 10px; fill: #212529; } .axis-label { font-size: 11px; fill: #212529; text-anchor: middle; } .grid .tick { stroke: #dee2e6; stroke-opacity: 0.6; } .grid path { stroke-width: 0; } .legend { font-size: 10px; fill: #212529; } .bar { opacity: 0.9; } .roc-curve { fill: none; stroke-width: 2px; } .reference-line { stroke: #adb5bd; stroke-width: 1px; stroke-dasharray: 4 2; } .auc-label { font-weight: bold; font-size: 11px; }`; clone.prepend(styleEl); return clone.outerHTML; } catch (e) { return `<p class="text-danger small">[Error embedding chart: ${e.message}]</p>`; } };
            ...
            if (config.INCLUDE_AS_VS_T2_COMPARISON_TABLE && statsDataForCurrentKollektiv?.comparisonASvsT2Applied) { html += `<h2>Statistical Comparison: AS vs. T2 (${appliedCriteriaDisplayName})</h2><table><thead><tr><th>Test</th><th>Statistic</th><th>p-Value</th><th>Method</th></tr></thead><tbody>`; const v = statsDataForCurrentKollektiv.comparisonASvsT2Applied; html += `<tr><td>Accuracy (McNemar)</td><td>${window.utils.formatNumber(v?.mcnemar?.statistic, 3, na, true)} (df=${v?.mcnemar?.df || na})</td><td>${window.utils.getPValueText(v?.mcnemar?.pValue, false)} ${window.utils.getStatisticalSignificanceSymbol(v?.mcnemar?.pValue)}</td><td>${v?.mcnemar?.method || na}</td></tr>`; html += `<tr><td>AUC (DeLong)</td><td>Z=${window.utils.formatNumber(v?.delong?.Z, 3, na, true)}</td><td>${window.utils.getPValueText(v?.delong?.pValue, false)} ${window.utils.getStatisticalSignificanceSymbol(v?.delong?.pValue)}</td><td>${v?.delong?.method || na}</td></tr>`; html += `</tbody></table>`; }
            ...
            if (config.INCLUDE_ASSOCIATIONS_TABLE && statsDataForCurrentKollektiv?.associationsApplied && Object.keys(statsDataForCurrentKollektiv.associationsApplied).length > 0) { html += `<h2>Association with N-Status</h2><table><thead><tr><th>Feature</th><th>OR (95% CI)</th><th>RD (%) (95% CI)</th><th>Phi</th><th>p-Value</th><th>Test</th></tr></thead><tbody>`; const a = statsDataForCurrentKollektiv.associationsApplied; const fRowAssoc = (nm, obj) => { if (!obj) return ''; const orS = window.utils.formatCI(obj.or?.value, obj.or?.ci?.lower, obj.or?.ci?.upper, 2, false, na); const rdV = window.utils.formatNumber(obj.rd?.value !== null && !isNaN(obj.rd?.value) ? obj.rd.value * 100 : NaN, 1, na, true); const rdL = window.utils.formatNumber(obj.rd?.ci?.lower !== null && !isNaN(obj.rd?.ci?.lower) ? obj.rd.ci.lower * 100 : NaN, 1, na, true); const rdU = window.utils.formatNumber(obj.rd?.ci?.upper !== null && !isNaN(obj.rd?.ci?.upper) ? obj.rd.ci.upper * 100 : NaN, 1, na, true); const rdS = rdV !== na ? `${rdV}% (${rdL}% - ${rdU}%)` : na; const phiS = window.utils.formatNumber(obj.phi?.value, 2, na, true); const pS = window.utils.getPValueText(obj.pValue, false) + ' ' + window.utils.getStatisticalSignificanceSymbol(obj.pValue); const tN = obj.testName || na; return `<tr><td>${nm}</td><td>${orS}</td><td>${rdS}</td><td>${phiS}</td><td>${pS}</td><td>${tN}</td></tr>`; }; html += fRowAssoc('AS Positive', a?.as); if (a?.size_mwu) html += `<tr><td>${a.size_mwu.featureName || 'LN Size (Median Comp.)'}</td><td>${na}</td><td>${na}</td><td>${na}</td><td>${window.utils.getPValueText(a.size_mwu.pValue, false)} ${window.utils.getStatisticalSignificanceSymbol(a.size_mwu.pValue)}</td><td>${a.size_mwu.testName || na}</td></tr>`; ['size', 'shape', 'border', 'homogeneity', 'signal'].forEach(k => { if (a && a[k]) { const isActive = criteria[k]?.active === true; html += fRowAssoc(a[k].featureName + (isActive ? '' : ' (inactive)'), a[k]); } }); html += `</tbody></table>`; }
            ...
            if (config.INCLUDE_BRUTEFORCE_BEST_RESULT && currentKollektivBfResult && Object.values(currentKollektivBfResult)[0]?.bestResult) {
                ...
                html += `<li><strong>Optimized Metric:</strong> ${bestResultForMetric.metric}</li><li><strong>Best Value:</strong> ${window.utils.formatNumber(best.metricValue, 4, na, true)}</li><li><strong>Logic:</strong> ${best.logic?.toUpperCase()}</li><li><strong>Criteria:</strong> ${formatCriteriaFunc(best.criteria, best.logic)}</li></ul><p class="small text-muted">Cohort N=${window.utils.formatNumber(bestResultForMetric.nTotal, 0, na)} (N+: ${window.utils.formatNumber(bestResultForMetric.nPlus, 0, na)}, N-: ${window.utils.formatNumber(bestResultForMetric.nMinus, 0, na)})</p></div>`; 
            }
    ...
    async function _exportChartsZip(selector, typeKey, kollektiv, format) {
        if (!window.JSZip) {
            window.uiManager.showToast("JSZip library not loaded for chart export.", "danger");
            return;
        }
        const zip = new JSZip();
        let filesAdded = 0;
        const chartElements = document.querySelectorAll(`${selector} .chart-container svg`);

        for (const svgElement of chartElements) {
            const chartContainer = svgElement.closest('.chart-container');
            ...
            let blob = null;
            let currentFilenameKey;
            let currentExt;
            let currentMimeType;

            if (format === 'png') {
                blob = await convertSvgToPngBlob(svgElement);
                currentFilenameKey = 'CHART_SINGLE_PNG';
                currentExt = 'png';
                currentMimeType = 'image/png';
            } else if (format === 'svg') {
                blob = await convertSvgToSvgBlob(svgElement);
                currentFilenameKey = 'CHART_SINGLE_SVG';
                currentExt = 'svg';
                currentMimeType = 'image/svg+xml;charset=utf-8';
            } else {
                continue;
            }

            if (blob) {
                const filename = generateFilename(currentFilenameKey, kollektiv, currentExt, { chartName: chartName });
                if (blob.size > 0) {
        ...
        if (filesAdded > 0) {
            ...
            if (downloadFile(content, zipFilename, "application/zip")) {
                window.uiManager.showToast(`${filesAdded} chart(s) successfully exported as ${format.toUpperCase()} ZIP package.`, 'success');
            }
        } else {
            window.uiManager.showToast(`No charts found or generated for ${format.toUpperCase()} ZIP package.`, 'warning');
        }
    }
    ...
    async function exportCategoryZip(category, data, bfResults, kollektiv, criteria, logic) {
         ...
         try {
             ...
             if (['png'].includes(category)) { await _exportChartsZip('#app-container', 'PNG_ZIP', kollektiv, 'png'); return; }
             if (['svg'].includes(category)) { await _exportChartsZip('#app-container', 'SVG_ZIP', kollektiv, 'svg'); return; }
    ...
    function exportComparisonData(actionId, comparisonData, kollektiv) {
        ...
        try {
            if (isAsPurView && actionId === 'download-performance-as-pur-csv') {
                 ...
                 const fVal = (v, d=1, useStd = false) => window.utils.formatNumber(v, d, na, useStd);
                 const rows = Object.entries(allStatsData).map(([key, stats]) => { let k = key.replace('stats',''); let dN = (k === 'Gesamt') ? 'Overall' : (k === 'SurgeryAlone') ? 'surgeryAlone' : 'neoadjuvantTherapy'; if (!stats || typeof stats.matrix !== 'object') return [window.utils.getCohortDisplayName(dN), 0, ...Array(24).fill(na), na]; const n = stats.matrix ? (stats.matrix.tp + stats.matrix.fp + stats.matrix.fn + stats.matrix.tn) : 0; const fRowData = (m, metric_k) => { const dig = (metric_k === 'auc' || metric_k === 'f1' || metric_k === 'balAcc') ? 3 : 1; return [fVal(m?.value, dig, true), fVal(m?.ci?.lower, dig, true), fVal(m?.ci?.upper, dig, true)]; }; return [ window.utils.getCohortDisplayName(dN), n, ...fRowData(stats.sens, 'sens'), ...fRowData(stats.spec, 'spec'), ...fRowData(stats.ppv, 'ppv'), ...fRowData(stats.npv, 'npv'), ...fRowData(stats.acc, 'acc'), ...fRowData(stats.balAcc, 'balAcc'), ...fRowData(stats.f1, 'f1'), ...fRowData(stats.auc, 'auc'), stats.sens?.method || na ]; });
