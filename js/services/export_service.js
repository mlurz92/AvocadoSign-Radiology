window.exportService = (() => {
    ...
    function generateStatistikCSVString(stats, kollektiv, criteria, logic) {
        ...
        try {
            ...
            csvData.push(['Angewandte T2 Kriterien', formatCriteriaFunc(criteria, logic, false)]); // NEU: false für lange Version
    ...
    function generateMarkdownTableString(dataOrStats, tableType, kollektiv, criteria = null, logic = null, options = {}) {
        try {
            ...
            const na = window.APP_CONFIG.NA_PLACEHOLDER;
            const formatCriteriaFunc = typeof window.studyT2CriteriaManager !== 'undefined' ? window.studyT2CriteriaManager.formatCriteriaForDisplay : (c, l) => 'N/A'; 
            const t2CriteriaLabelShort = options.t2CriteriaLabelShort || window.APP_CONFIG.UI_TEXTS.labels.appliedT2Short; // NEU: Standard von config holen
    ...
    function generateComprehensiveReportHTML(data, allBruteForceResults, kollektiv, criteria, logic) {
        try {
            ...
            const formatCriteriaFunc = typeof window.studyT2CriteriaManager !== 'undefined' ? window.studyT2CriteriaManager.formatCriteriaForDisplay : (c, l) => 'N/A';
            const kollektivName = getCohortDisplayName(kollektiv); const timestamp = new Date().toLocaleString('en-US', { dateStyle: 'long', timeStyle: 'medium'}); const criteriaString = formatCriteriaFunc(criteria, logic, false); // NEU: false für lange Version
            const appliedCriteriaDisplayName = window.APP_CONFIG.UI_TEXTS.labels.appliedT2Short; // NEU: Holen des kurzen Labels von config

            let appliedCriteriaFullDisplayName = window.APP_CONFIG.UI_TEXTS.labels.appliedT2Full.replace('[CRITERIA]', criteriaString); // NEU: Erstellen des vollständigen Labels
            ...
            if (config.INCLUDE_T2_PERFORMANCE_TABLE && statsDataForCurrentKollektiv?.performanceT2Applied) { html += addPerfSectionHTML(`Diagnostic Performance: T2 ('${appliedCriteriaFullDisplayName}' vs. N)`, statsDataForCurrentKollektiv.performanceT2Applied); } // NEU: appliedCriteriaFullDisplayName
            if (config.INCLUDE_AS_VS_T2_COMPARISON_TABLE && statsDataForCurrentKollektiv?.comparisonASvsT2Applied) { html += `<h2>Statistical Comparison: AS vs. T2 ('${appliedCriteriaFullDisplayName}')</h2><table><thead><tr><th>Test</th><th>Statistic</th><th>p-Value</th><th>Method</th></tr></thead><tbody>`; const v = statsDataForCurrentKollektiv.comparisonASvsT2Applied; html += `<tr><td>Accuracy (McNemar)</td><td>${formatNumber(v?.mcnemar?.statistic, 3, na, true)} (df=${v?.mcnemar?.df || na})</td><td>${getPValueText(v?.mcnemar?.pValue, false)} ${getStatisticalSignificanceSymbol(v?.mcnemar?.pValue)}</td><td>${v?.mcnemar?.method || na}</td></tr>`; html += `<tr><td>AUC (DeLong)</td><td>Z=${formatNumber(v?.delong?.Z, 3, na, true)}</td><td>${getPValueText(v?.delong?.pValue, false)} ${getStatisticalSignificanceSymbol(v?.delong?.pValue)}</td><td>${v?.delong?.method || na}</td></tr>`; html += `</tbody></table>`; }
            if (config.INCLUDE_AS_VS_T2_COMPARISON_CHART) { const chartKey = Object.keys(chartSVGs).find(k => k.startsWith('comp-chart')); if(chartSVGs[chartKey]) { html += `<div class="chart-container"><h3>Comparison of Selected Metrics (AS vs T2 - '${appliedCriteriaFullDisplayName}')</h3>${chartSVGs[chartKey]}</div>`; } }
             ...
             if (['all', 'md'].includes(category)) {
                 ...
                 if (window.PUBLICATION_CONFIG && window.state && window.publicationService && window.APP_CONFIG) {
                     const commonDataForPub = { 
                        ...
                        appliedCriteria: criteria, // Add applied criteria
                        appliedLogic: logic, // Add applied logic
                        appliedT2CriteriaDisplayShort: window.studyT2CriteriaManager.formatCriteriaForDisplay(criteria, logic, true),
                        appliedT2CriteriaDisplayFull: window.studyT2CriteriaManager.formatCriteriaForDisplay(criteria, logic, false)
                    };
    ...
    function exportComparisonData(actionId, comparisonData, kollektiv) {
        ...
        // NEU: Dynamische T2-Kriterien für exportComparisonData
        ...
        options.appliedT2CriteriaDisplayShort = window.studyT2CriteriaManager.formatCriteriaForDisplay(appliedCriteria, appliedLogic, true);
        options.appliedT2CriteriaDisplayFull = window.studyT2CriteriaManager.formatCriteriaForDisplay(appliedCriteria, appliedLogic, false);
        
        // Use dynamically generated string for T2 if it's the applied criteria
        if (comparisonCriteriaSet?.id === window.APP_CONFIG.SPECIAL_IDS.APPLIED_CRITERIA_STUDY_ID) {
            options.t2CriteriaLabelShort = options.appliedT2CriteriaDisplayShort;
            options.t2CriteriaLabelFull = options.appliedT2CriteriaDisplayFull;
        } else {
            options.t2CriteriaLabelShort = t2ShortName;
            options.t2CriteriaLabelFull = window.studyT2CriteriaManager.formatCriteriaForDisplay(comparisonCriteriaSet?.criteria, comparisonCriteriaSet?.logic, false);
        }
