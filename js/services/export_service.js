window.exportService = (() => {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const canvas = await html2canvas(tableElement, {
                    scale: window.APP_CONFIG.EXPORT_SETTINGS.TABLE_PNG_EXPORT_SCALE, // Use config value
                    useCORS: true,
                ...
                window.uiManager.showToast('Could not load rendering library for table export.', 'danger');
    ...
    function generateStatistikCSVString(stats, kollektiv, criteria, logic) {
        ...
        const csvData = []; const na = window.APP_CONFIG.NA_PLACEHOLDER; // Use global placeholder
    ...
    function generateBruteForceTXTString(resultsData) {
        ...
        try {
            ...
            report += `   - Total Patients: ${formatNumber(nTotal, 0, window.APP_CONFIG.NA_PLACEHOLDER)}\r\n`; // Use global placeholder
            report += `   - N+ Patients: ${formatNumber(nPlus, 0, window.APP_CONFIG.NA_PLACEHOLDER)}\r\n`;   // Use global placeholder
            report += `   - N- Patients: ${formatNumber(nMinus, 0, window.APP_CONFIG.NA_PLACEHOLDER)}\r\n`;  // Use global placeholder
            report += `Optimized Target Metric: ${metric}\r\n`;
            report += `Total Duration: ${formatNumber((duration || 0) / 1000, 1, window.APP_CONFIG.NA_PLACEHOLDER, true)} seconds\r\n`; // Use global placeholder
            report += `Combinations Tested: ${formatNumber(totalTested, 0, window.APP_CONFIG.NA_PLACEHOLDER, true)}\r\n`; // Use global placeholder
            report += `==================================================\r\n\r\n`;
            report += `--- Best Result ---\r\n`;
            report += `Logic: ${bestResult.logic.toUpperCase()}\r\n`;
            report += `Criteria: ${formatCriteriaFunc(bestResult.criteria, best.logic)}\r\n`;
            report += `Achieved ${metric}: ${formatNumber(best.metricValue, 4, window.APP_CONFIG.NA_PLACEHOLDER, true)}\r\n\r\n`; // Use global placeholder
            ...
            for (let i = 0; i < results.length; i++) {
                ...
                report += `${String(currentRank).padEnd(4)} | ${formatNumber(result.metricValue, 4, window.APP_CONFIG.NA_PLACEHOLDER, true).padEnd(12)} | ${result.logic.toUpperCase().padEnd(5)} | ${formatCriteriaFunc(result.criteria, result.logic)}\r\n`; // Use global placeholder
    ...
    function generateComprehensiveReportHTML(data, allBruteForceResults, kollektiv, criteria, logic) {
        try {
            ...
            const formatCriteriaFunc = typeof window.studyT2CriteriaManager !== 'undefined' ? window.studyT2CriteriaManager.formatCriteriaForDisplay : (c, l) => 'N/A';
            const kollektivName = getCohortDisplayName(kollektiv); const timestamp = new Date().toLocaleString('en-US', { dateStyle: 'long', timeStyle: 'medium'}); const criteriaString = formatCriteriaFunc(criteria, logic); const appliedCriteriaDisplayName = window.APP_CONFIG.SPECIAL_IDS.APPLIED_CRITERIA_DISPLAY_NAME;
            ...
            if (config.INCLUDE_AS_VS_T2_COMPARISON_TABLE && statsDataForCurrentKollektiv?.comparisonASvsT2Applied) { html += `<h2>Statistical Comparison: AS vs. T2 ('${appliedCriteriaDisplayName}')</h2><table><thead><tr><th>Test</th><th>Statistic</th><th>p-Value</th><th>Method</th></tr></thead><tbody>`; const v = statsDataForCurrentKollektiv.comparisonASvsT2Applied; const na = window.APP_CONFIG.NA_PLACEHOLDER; html += `<tr><td>Accuracy (McNemar)</td><td>${formatNumber(v?.mcnemar?.statistic, 3, na, true)} (df=${v?.mcnemar?.df || na})</td><td>${getPValueText(v?.mcnemar?.pValue, false)} ${getStatisticalSignificanceSymbol(v?.mcnemar?.pValue)}</td><td>${v?.mcnemar?.method || na}</td></tr>`; html += `<tr><td>AUC (DeLong)</td><td>Z=${formatNumber(v?.delong?.Z, 3, na, true)}</td><td>${getPValueText(v?.delong?.pValue, false)} ${getStatisticalSignificanceSymbol(v?.delong?.pValue)}</td><td>${v?.delong?.method || na}</td></tr>`; html += `</tbody></table>`; }
            if (config.INCLUDE_AS_VS_T2_COMPARISON_CHART) { const chartKey = Object.keys(chartSVGs).find(k => k.startsWith('comp-comp-chart')); if(chartSVGs[chartKey]) { html += `<div class="chart-container"><h3>Comparison of Selected Metrics (AS vs T2 - '${appliedCriteriaDisplayName}')</h3>${chartSVGs[chartKey]}</div>`; } }
            if (config.INCLUDE_ASSOCIATIONS_TABLE && statsDataForCurrentKollektiv?.associationsApplied && Object.keys(statsDataForCurrentKollektiv.associationsApplied).length > 0) { html += `<h2>Association with N-Status</h2><table><thead><tr><th>Feature</th><th>OR (95% CI)</th><th>RD (%) (95% CI)</th><th>Phi</th><th>p-Value</th><th>Test</th></tr></thead><tbody>`; const a = statsDataForCurrentKollektiv.associationsApplied; const na = window.APP_CONFIG.NA_PLACEHOLDER; const fRowAssoc = (nm, obj) => { if (!obj) return ''; const orS = formatCI(obj.or?.value, obj.or?.ci?.lower, obj.or?.ci?.upper, 2, false, na); const rdV = formatNumber(obj.rd?.value !== null && !isNaN(obj.rd?.value) ? obj.rd.value * 100 : NaN, 1, na, true); const rdL = formatNumber(obj.rd?.ci?.lower !== null && !isNaN(obj.rd?.ci?.lower) ? obj.rd.ci.lower * 100 : NaN, 1, na, true); const rdU = formatNumber(obj.rd?.ci?.upper !== null && !isNaN(obj.rd?.ci?.upper) ? obj.rd.ci.upper * 100 : NaN, 1, na, true); const rdS = rdV !== na ? `${rdV}% (${rdL}% - ${rdU}%)` : na; const phiS = formatNumber(obj.phi?.value, 2, na, true); const pS = getPValueText(obj.pValue, false) + ' ' + getStatisticalSignificanceSymbol(obj.pValue); const tN = obj.testName || na; return `<tr><td>${nm}</td><td>${orS}</td><td>${rdS}</td><td>${phiS}</td><td>${pS}</td><td>${tN}</td></tr>`; }; html += fRowAssoc('AS Positive', a?.as); if (a?.size_mwu) html += `<tr><td>${a.size_mwu.featureName || 'LN Size (Median Comp.)'}</td><td>${na}</td><td>${na}</td><td>${na}</td><td>${getPValueText(a.size_mwu.pValue, false)} ${getStatisticalSignificanceSymbol(a.size_mwu.pValue)}</td><td>${a.size_mwu.testName || na}</td></tr>`; ['size', 'shape', 'border', 'homogeneity', 'signal'].forEach(k => { if (a && a[k]) { const isActive = criteria[k]?.active === true; html += fRowAssoc(a[k].featureName + (isActive ? '' : ' (inactive)'), a[k]); } }); html += `</tbody></table>`; }
            const currentKollektivBfResult = allBruteForceResults ? allBruteForceResults[kollektiv] : null;
            if (config.INCLUDE_BRUTEFORCE_BEST_RESULT && currentKollektivBfResult?.results && currentKollektivBfResult.results.length > 0 && currentKollektivBfResult.bestResult) { html += `<h2>Best Brute-Force Result (for Cohort: ${kollektivName})</h2><div class="meta-info"><ul>`; const best = currentKollektivBfResult.bestResult; html += `<li><strong>Optimized Metric:</strong> ${currentKollektivBfResult.metric}</li><li><strong>Best Value:</strong> ${formatNumber(best.metricValue, 4, window.APP_CONFIG.NA_PLACEHOLDER, true)}</li><li><strong>Logic:</strong> ${best.logic?.toUpperCase()}</li><li><strong>Criteria:</strong> ${formatCriteriaFunc(best.criteria, best.logic)}</li></ul><p class="small text-muted">Cohort N=${formatNumber(currentKollektivBfResult.nTotal, 0, window.APP_CONFIG.NA_PLACEHOLDER)} (N+: ${formatNumber(currentKollektivBfResult.nPlus, 0, window.APP_CONFIG.NA_PLACEHOLDER)}, N-: ${formatNumber(currentKollektivBfResult.nMinus, 0, window.APP_CONFIG.NA_PLACEHOLDER)})</p></div>`; }
     ...
     async function exportCategoryZip(category, data, bfResults, kollektiv, criteria, logic) {
         ...
         try {
             ...
             if (['all-zip', 'md-zip'].includes(category)) {
                 ...
                 if (data && data.length > 0) addFile(generateFilename('DATEN_MD', kollektiv, 'md'), generateMarkdownTableString(window.dataProcessor.filterDataByCohort(data, kollektiv), 'daten', kollektiv));
                 if (data && data.length > 0) addFile(generateFilename('AUSWERTUNG_MD', kollektiv, 'md'), generateMarkdownTableString(window.t2CriteriaManager.evaluateDataset(window.dataProcessor.filterDataByCohort(data, kollektiv), criteria, logic), 'auswertung', kollektiv, criteria, logic));

                 if (window.PUBLICATION_CONFIG && window.state && window.publicationService && window.APP_CONFIG) { // Added APP_CONFIG check
                     ...
                     window.PUBLICATION_CONFIG.sections.forEach(mainSection => {
                         mainSection.subSections.forEach(subSection => {
                            const mdContent = window.publicationService.generateSectionHTML(subSection.id, lang, statsDataForAllKollektive, commonDataForPub);
    ...
    function exportComparisonData(actionId, comparisonData, kollektiv) {
        let content = null, filenameKey = null, extension = null, mimeType = null, options = {}, success = false; const na = window.APP_CONFIG.NA_PLACEHOLDER;
        ...
        if (comparisonData.t2ShortName) options.t2CriteriaLabelShort = comparisonData.t2ShortName;
        if (comparisonCriteriaSet?.description) options.t2CriteriaLabelFull = window.studyT2CriteriaManager.formatCriteriaForDisplay(comparisonCriteriaSet.criteria, comparisonCriteriaSet.logic);

        try {
            if (isAsPurView && actionId === 'download-performance-as-pur-csv') {
                 const allStatsData = { statsGesamt: comparisonData.statsGesamt, statsSurgeryAlone: comparisonData.statsSurgeryAlone, statsNeoadjuvantTherapy: comparisonData.statsNeoadjuvantTherapy }; const headers = ['Cohort', 'N', 'Sens', 'Sens CI Low', 'Sens CI High', 'Spec', 'Spec CI Low', 'Spec CI High', 'PPV', 'PPV CI Low', 'PPV CI High', 'NPV', 'NPV CI Low', 'NPV CI High', 'Acc', 'Acc CI Low', 'Acc CI High', 'BalAcc', 'BalAcc CI Low', 'BalAcc CI High', 'F1', 'F1 CI Low', 'F1 CI High', 'AUC', 'AUC CI Low', 'AUC CI High', 'CI Method']; const fVal = (v, d=1, useStd = false) => formatNumber(v, d, na, useStd);
                 const rows = Object.entries(allStatsData).map(([key, stats]) => { let k = key.replace('stats',''); let dN = (k === 'Gesamt') ? 'Overall' : (k === 'SurgeryAlone') ? 'surgeryAlone' : 'neoadjuvantTherapy'; if (!stats || typeof stats.matrix !== 'object') return [getCohortDisplayName(dN), 0, ...Array(24).fill(na), na]; const n = stats.matrix ? (stats.matrix.tp + stats.matrix.fp + stats.matrix.fn + stats.matrix.tn) : 0; const fRowData = (m, metric_k) => { const dig = (metric_k === 'auc') ? 3 : ((metric_k === 'f1') ? 3 : 1); return [fVal(m?.value, dig, true), fVal(m?.ci?.lower, dig, true), fVal(m?.ci?.upper, dig, true)]; }; return [ getCohortDisplayName(dN), n, ...fRowData(stats.sens, 'sens'), ...fRowData(stats.spec, 'spec'), ...fRowData(stats.ppv, 'ppv'), ...fRowData(stats.npv, 'npv'), ...fRowData(stats.acc, 'acc'), ...fRowData(stats.balAcc, 'balAcc'), ...fRowData(stats.f1, 'f1'), ...fRowData(stats.auc, 'auc'), stats.sens?.method || na ]; });
