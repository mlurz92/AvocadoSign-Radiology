const exportService = (() => {
    ...
    function generateComprehensiveReportHTML(data, allBruteForceResults, kollektiv, criteria, logic) {
        try {
            ...
            const formatCriteriaFunc = typeof studyT2CriteriaManager !== 'undefined' ? studyT2CriteriaManager.formatCriteriaForDisplay : (c, l) => 'N/A';
            const kollektivName = getCohortDisplayName(kollektiv); const timestamp = new Date().toLocaleString('en-US', { dateStyle: 'long', timeStyle: 'medium'}); const criteriaString = formatCriteriaFunc(criteria, logic); const appliedCriteriaDisplayName = APP_CONFIG.SPECIAL_IDS.APPLIED_CRITERIA_DISPLAY_NAME;
    ...
    function exportComparisonData(actionId, comparisonData, kollektiv) {
        ...
        if (!comparisonData) { uiManager.showToast("No data available for comparison export.", "warning"); return; }
        ...
        try {
            if (isAsPurView && actionId === 'download-performance-as-pur-csv') {
                 ...
                 content = Papa.unparse([headers, ...rows], { delimiter: APP_CONFIG.EXPORT_SETTINGS.CSV_DELIMITER || ";" }); filenameKey = 'COMP_AS_PERF_CSV'; extension = 'csv'; mimeType = 'text/csv;charset=utf-8;';
            ...
            uiManager.showToast(`Error during comparison export (${actionId}).`, "danger");
            return;
        }

        if(content !== null && filenameKey && extension && mimeType) { const filename = generateFilename(filenameKey, kollektiv, extension, options); if(downloadFile(content, filename, mimeType)) uiManager.showToast(`Comparison data (${extension}) exported: ${filename}`, 'success'); }
